...
/**
 * 用于节点对象转换成SensorThings观测数据对象的具体策略
 */
@Slf4j
public class NodeMapStrategy implements SensorDataStrategy {
	...
	public List<Observation> mapObservation(VisualObject object, LocalDateTime time) {
		...
		try {
			...
			for (String key : computedProperties.keySet()) {
				...
				observation.setName("The observation record of node " + object.getId());
	...
	public List<ObservedProperty> mapObservedProperty(VisualObject object) {
		...
		try {
			...
			for (String key : computedProperties.keySet()) {
				...
				observation.setDefinition(Node.class.getName());
				observation.setDescription("The " + key + " property for Node.");
				observations.add(observation);
			}
		} catch (Exception e) {
