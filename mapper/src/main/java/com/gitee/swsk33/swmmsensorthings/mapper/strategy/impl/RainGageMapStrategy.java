...
/**
 * 用于雨量计对象转换成SensorThings观测数据对象的具体策略
 */
@Slf4j
public class RainGageMapStrategy implements SensorDataStrategy {
	...
	public List<Observation> mapObservation(VisualObject object, LocalDateTime time) {
		...
		try {
			...
			for (String key : computedProperties.keySet()) {
				...
				observation.setName("The observation record of rain gage " + object.getId());
	...
	public List<ObservedProperty> mapObservedProperty(VisualObject object) {
		...
		try {
			...
			for (String key : computedProperties.keySet()) {
				...
				observation.setDefinition(RainGage.class.getName());
				observation.setDescription("The " + key + " property for RainGage.");
				observations.add(observation);
			}
		} catch (Exception e) {
