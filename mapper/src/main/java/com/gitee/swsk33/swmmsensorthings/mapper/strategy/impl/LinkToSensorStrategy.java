	...
	public Map<String, Sensor> createSensors(VisualObject object) {
		...
		try {
			...
			for (String key : computedProperties.keySet()) {
				...
				sensor.setDescription("The virtual sensor of the link " + object.getId() + ", which observes the " + key + " property");
				sensor.setMetadata(Link.class.getSimpleName() + "." + key);
				sensor.setEncodingType(EncodingType.JSON);
				virtualSensors.put(key, sensor);
			}
		} catch (Exception e) {
			log.error("读取链接对象计算属性出错！");
