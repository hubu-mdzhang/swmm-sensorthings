...
/**
 * 将链接对象转换成SensorThings观测对象的具体策略
 */
@Slf4j
public class LinkMapStrategy implements SensorDataStrategy {
	...
	public List<Observation> mapObservation(VisualObject object, LocalDateTime time) {
		...
		try {
			...
			for (String key : computedProperties.keySet()) {
				...
				observation.setName("The observation record of link " + object.getId());
	...
	public List<ObservedProperty> mapObservedProperty(VisualObject object) {
		...
		try {
			...
			for (String key : computedProperties.keySet()) {
				...
				observation.setDefinition(Link.class.getName());
				observation.setDescription("The " + key + " property for Link.");
				observations.add(observation);
			}
		} catch (Exception e) {
