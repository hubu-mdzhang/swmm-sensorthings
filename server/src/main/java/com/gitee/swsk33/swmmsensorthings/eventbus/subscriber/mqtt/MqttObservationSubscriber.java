...
public class MqttObservationSubscriber implements IMqttMessageListener {
	 ...
	 * @param swmm 输入SWMM对象
	 */
	public MqttObservationSubscriber(SWMM swmm) {
		// 初始化数据缓存池
	...
	public void messageArrived(String topic, MqttMessage message) {
		// 解析消息
		// 要求发布的Observation对象的datastream属性不能为空，且datastream必须包含id和name属性（且不为空）
		Observation data = JSON.parseObject(message.getPayload(), Observation.class);
		log.info("MQTT订阅到：{}", data);
		// 传递到数据缓存池
